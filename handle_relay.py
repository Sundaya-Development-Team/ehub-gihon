...
def handle_selection():
    if PANEL2_TYPE == "new":
        ...
        if float(red.hget("sensor_arus", "load3")) > 0:
            ...
            obl_state = False
        other_state = False
    return bts_on, bts_off, vsat_on, vsat_off, bts_state, vsat_state, other_state, obl_state

if __name__ == '__main__':
    try:
        bts_on, bts_off, vsat_on, vsat_off, bts_state, vsat_state, other_state, obl_state = handle_selection()
        ...
        log.info(min_volt)
        log.info(f'bts_state = {bts_state} vsat_state = {vsat_state} obl_state = {obl_state}')
        if avg_volt <= 4673:
            log.info('bts cutoff')
